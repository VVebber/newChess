...
int main(){
    ...
    std::vector<std::string> FileNamesChessBoard = { "images/ChessBoard/WhiteBlack_1.png","images/ChessBoard/WhiteBlack_2.png" };
    ...
    for (int i = 0; i < 8 * QuantityPlayrs; i++) {
        std::string posit = (i < 8) ? "up" : "down";
        Pawn pawn(i,posit, "pawn", TxPawn);
        Pawns.push_back(pawn);
    }

    std::vector<Queen> Queens; 
    for (int i = 0; i < 1 * QuantityPlayrs; i++) {
        std::string posit = (i < 1) ? "up" : "down";
        Queen queen(i, posit, "queen", TxQueen);
    ...
    for (int i = 0; i < 1 * QuantityPlayrs; i++) {
        std::string posit = (i < 1) ? "up" : "down";
        King king(i, posit, "king", TxKing);
        Kings.push_back(king);
    }

    std::vector<Rook> Rooks;
    for (int i = 0; i < 2 * QuantityPlayrs; i++) {
        std::string posit = (i < 2) ? "up" : "down";
        Rook rook(i, posit, "rook", TxRook);
    ...
    for (int i = 0; i < 2 * QuantityPlayrs; i++) {
        std::string posit = (i < 2) ? "up" : "down";
        Knight knight(i, posit, "knights", TxKnight);
    ...
    for (int i = 0; i < 2 * QuantityPlayrs; i++) {
        std::string posit = (i < 2) ? "up" : "down";
        Bishop bishop(i, posit, "bishops", TxBishop);
